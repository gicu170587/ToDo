<template>
    <div>
            ...
            <b-alert variant="success" v-if="registeredUser"  show style="color:#222;padding:10px">Thank you {{registeredUser.name}}.You can now login</b-alert>
    ...
    <b-card-text style="max-width:80%; margin:auto;text-align:left; ">
                    <form @submit.prevent="authenticate">
                        ...
                        <div class="form-group row" :class="{ 'hasError': $v.formLogin.email.$error }">
                           ...
                           <span v-if="!$v.formLogin.email.required" class="alert" role="alert">Email is required</span>
                           <span v-if="!$v.formLogin.email.email" class="alert" role="alert">Email not valid</span>
                             </div>
                        </div>
                         <div class="form-group row" :class="{ 'hasError': $v.formLogin.password.$error }">
                            ...
                            <span v-if="$v.formLogin.password.$error" class="alert" role="alert">Password is required</span>
                        </div>
